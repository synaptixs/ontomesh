...
The published image at `ghcr.io/synaptixs/ontomesh:3.10.0` is multi-arch (`linux/amd64` + `linux/arm64`), runs as a non-root user, and bundles every default extra needed to open the wizard.
...
```bash
docker buildx imagetools inspect ghcr.io/synaptixs/ontomesh:3.10.0 \
  --format '{{ json .SBOM }}'
```
