...
Module layout
-------------
``ontoforge``               — version, package metadata
``ontoforge.cli``           — ``ontoforge`` console script (full pipeline)
``ontoforge.wizard_entry``  — ``ontoforge-wizard`` console script (Flask UI)
``ontoforge.onboard_entry`` — ``ontoforge-onboard`` console script (REPL wizard)
...
``ontoforge.<x>`` is a long-term commitment; private helpers stay under
